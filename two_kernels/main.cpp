...
void PrintUsage(const char* argv0) {
    std::cerr
        << "Usage:\n"
        << "  " << argv0 << " <mosaic_mlir_file_1> <mosaic_mlir_file_2> <libtpu_so> [<output_blob_path>]\n"
...
int main(int argc, char** argv) {
    ...
    if (load_blob) {
        ...
        blob_out_path =
            (positional.size() == 4) ? positional[3] : nullptr;

        libtpu_path = positional[2];
    }
    ...
    if (!load_blob) {
        ...
        if (trivial) {
            ...
            stablehlo = WrapMosaicInStableHlo(mosaic_text_1, mosaic_text_2);
            std::cout << "[5] Loaded 2 Mosaic representations (" << mosaic_text_1.size() + mosaic_text_2.size()
    ...
    std::cout << "\nExpected:\n  ";
    for (int i = 0; i < 8; ++i) {
        std::cout << (host_first[i] + host_second[i] - host_third[i]) << " ";
    }
    std::cout << "\n";
