...
int main(int argc, char** argv) {
    ...
    std::vector<int32_t> host_lhs(N), host_rhs(N);
    for (size_t i = 0; i < N; ++i) {
        host_lhs[i] = static_cast<int32_t>(i);
        host_rhs[i] = 1;
    }
    ...
    auto upload = [&](const std::vector<int32_t>& host_data)
        -> PJRT_Buffer* {
    ...
    std::vector<int32_t> host_out(N);
    {
        ...
        args.dst_size = host_out.size() * sizeof(int32_t);
