...
std::string WrapMosaicInStableHlo(const std::string& mosaic_text) {
    ...
    return
        ...
        "    } : (tensor<128xi16>, tensor<128xi16>) -> tensor<128xi16>\n"
        "    return %out : tensor<128xi16>\n"
        "  }\n"
        "}\n";
}
...
int main(int argc, char** argv) {
    ...
    std::vector<int16_t> host_lhs(N), host_rhs(N);
    for (size_t i = 0; i < N; ++i) {
        host_lhs[i] = static_cast<int16_t>(i);
        host_rhs[i] = 1;
    }

    // ---- 7.a. Get Memory -----------------------------------
    PJRT_Device_DefaultMemory_Args mem_args = {
        .struct_size = PJRT_Device_DefaultMemory_Args_STRUCT_SIZE,
        .extension_start = nullptr,
        .device = device,
    };
    api->PJRT_Device_DefaultMemory(&mem_args);
    PJRT_Memory* memory = mem_args.memory;

    // ---- 8. Upload inputs to device -----------------------------------
    auto upload = [&](const std::vector<int16_t>& host_data)
        -> PJRT_Buffer* {
        ...
        args.type = PJRT_Buffer_Type_S16;
        args.dims = dims;
        args.num_dims = 1;
    ...
    std::vector<int16_t> host_out(N);
    {
        ...
        args.dst_size = host_out.size() * sizeof(int16_t);
