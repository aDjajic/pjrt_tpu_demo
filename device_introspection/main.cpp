...
// ===--------------------------------------------------------------------=== //
// Device introspection: how many TPU devices/cores are visible.
// ===--------------------------------------------------------------------=== //


void PrintNamedValue(const PJRT_NamedValue& nv) {
    std::cout << "        " << std::string(nv.name, nv.name_size) << " = ";
    switch (nv.type) {
        ...
        case PJRT_NamedValue_kInt64List:
            ...
            for (size_t i = 0; i < nv.value_size; ++i) {
                if (i > 0) std::cout << ", ";
                std::cout << nv.int64_array_value[i];
            }
            std::cout << "]";
            break;
...
bool DescribeDevice(const PJRT_Api* api, PJRT_Device* device) {
    ...
    if (!CheckError(api, api->PJRT_Device_GetDescription(&desc_args),
                    "PJRT_Device_GetDescription")) {
        return false;
    }
    PJRT_DeviceDescription* description = desc_args.device_description;

    PJRT_DeviceDescription_Kind_Args kind_args{};
    kind_args.struct_size = PJRT_DeviceDescription_Kind_Args_STRUCT_SIZE;
    kind_args.device_description = description;
    if (!CheckError(api, api->PJRT_DeviceDescription_Kind(&kind_args),
    ...
    id_args.device_description = description;
    if (!CheckError(api, api->PJRT_DeviceDescription_Id(&id_args),
                    "PJRT_DeviceDescription_Id")) {
        return false;
    }

    std::cout << "    device[id=" << id_args.id << "] kind=\""
    ...
    attr_args.device_description = description;
    if (!CheckError(api, api->PJRT_DeviceDescription_Attributes(&attr_args),
                    "PJRT_DeviceDescription_Attributes")) {
        return false;
    }
    for (size_t i = 0; i < attr_args.num_attributes; ++i) {
        PrintNamedValue(attr_args.attributes[i]);
    }
    return true;
}
...
int main(int argc, char** argv) {
    ...
    // ---- 4b. Report total TPU device/core count and per-device details --
    {
        PJRT_Client_Devices_Args all_dev_args{};
        all_dev_args.struct_size = PJRT_Client_Devices_Args_STRUCT_SIZE;
        all_dev_args.client = client;
        if (!CheckError(api, api->PJRT_Client_Devices(&all_dev_args),
                        "PJRT_Client_Devices")) {
            return 1;
        }

        PJRT_Client_AddressableDevices_Args addr_dev_args{};
        addr_dev_args.struct_size =
            PJRT_Client_AddressableDevices_Args_STRUCT_SIZE;
        addr_dev_args.client = client;
        if (!CheckError(api,
                        api->PJRT_Client_AddressableDevices(&addr_dev_args),
                        "PJRT_Client_AddressableDevices")) {
            return 1;
        }

        std::cout << "[4b] TPU topology: " << all_dev_args.num_devices
                  << " device(s)/core(s) total, "
                  << addr_dev_args.num_addressable_devices
                  << " addressable from this process\n";
        std::cout << (all_dev_args.num_devices > 1
                          ? "     -> multi-core/multi-chip TPU detected\n"
                          : "     -> single TPU core visible\n");

        for (size_t i = 0; i < addr_dev_args.num_addressable_devices; ++i) {
            if (!DescribeDevice(api, addr_dev_args.addressable_devices[i]))
                return 1;
        }
    }
