...
std::string WrapMosaicInStableHlo(const std::string& mosaic_text1, const std::string& mosaic_text2, 
                                    const std::string& mosaic_text3) {
    const std::string body_b64_1 = Base64Encode(mosaic_text1);
    const std::string body_b64_2 = Base64Encode(mosaic_text2);
    const std::string body_b64_3 = Base64Encode(mosaic_text3);
    // Inner JSON; escape the embedded quotes for the MLIR string literal.
    const std::string json_1 =
        std::string("{\\\"custom_call_config\\\": {\\\"body\\\": \\\"")
        + body_b64_1 + "\\\","
        + std::string(" \\\"needs_layout_passes\\\": true, \\\"shape_invariant_numerics\\\": false}}");

    const std::string json_2 =
        std::string("{\\\"custom_call_config\\\": {\\\"body\\\": \\\"")
        + body_b64_2 + "\\\","
        + std::string(" \\\"needs_layout_passes\\\": true, \\\"shape_invariant_numerics\\\": false}}");

    const std::string json_3 =
        std::string("{\\\"custom_call_config\\\": {\\\"body\\\": \\\"")
        + body_b64_3 + "\\\","
        + std::string(" \\\"needs_layout_passes\\\": true, \\\"shape_invariant_numerics\\\": false}}");
    ...
    return
        ...
        "      %arg2: tensor<128xf32>\n"
        "  ) -> tensor<128xf32> {\n"
        "    %0 = stablehlo.custom_call @tpu_custom_call(%arg0, %arg1) {\n"
        "      backend_config = \"" + json_1 + "\",\n"
        ...
        "    %1 = stablehlo.custom_call @tpu_custom_call(%0, %arg2) {\n"
        "      backend_config = \"" + json_2 + "\",\n"
        "      operand_layouts = [dense<0> : tensor<1xindex>, dense<0> : tensor<1xindex>],\n"
        "      result_layouts = [dense<0> : tensor<1xindex>]"
        "    } : (tensor<128xf32>, tensor<128xf32>) -> tensor<128xf32>\n"
        "    %2 = stablehlo.custom_call @tpu_custom_call(%0, %1) {\n"
        "      backend_config = \"" + json_3 + "\",\n"
        "      operand_layouts = [dense<0> : tensor<1xindex>, dense<0> : tensor<1xindex>],\n"
        "      result_layouts = [dense<0> : tensor<1xindex>]"
        "    } : (tensor<128xf32>, tensor<128xf32>) -> tensor<128xf32>\n"
...
void PrintUsage(const char* argv0) {
    std::cerr
        << "Usage:\n"
        << "  " << argv0 << " <mosaic_mlir_file_1> <mosaic_mlir_file_2> <mosaic_mlir_file_3> <libtpu_so> [<output_blob_path>]\n"
...
int main(int argc, char** argv) {
    ...
    const char* mosaic_path_1 = nullptr;
    const char* mosaic_path_2 = nullptr;
    const char* mosaic_path_3 = nullptr;
    const char* libtpu_path = nullptr;
    const char* blob_out_path = nullptr;
    const char* blob_in_path = nullptr;
    
    if ((positional.size() != 3 && positional.size() != 4 && !load_blob) || 
    ...
    if (load_blob) {
        ...
        mosaic_path_1 = positional[0];
        std::cout << "Mosaic MLIR: " << mosaic_path_1 << "\n";

        mosaic_path_2 = positional[1];
        std::cout << "Mosaic MLIR: " << mosaic_path_2 << "\n";

        mosaic_path_3 = positional[2];
        std::cout << "Mosaic MLIR: " << mosaic_path_3 << "\n";

        blob_out_path =
            (positional.size() == 5) ? positional[4] : nullptr;

        libtpu_path = positional[3];
    }
    ...
    if (!load_blob) {
        ...
        if (trivial) {
            ...
            std::string mosaic_text_1 = LoadTextFile(mosaic_path_1);
            std::string mosaic_text_2 = LoadTextFile(mosaic_path_2);
            std::string mosaic_text_3 = LoadTextFile(mosaic_path_3);
            if (mosaic_text_1.empty()) return 1;
            if (mosaic_text_2.empty()) return 1;
            if (mosaic_text_3.empty()) return 1;
            stablehlo = WrapMosaicInStableHlo(mosaic_text_1, mosaic_text_2, mosaic_text_3);
            std::cout << "[5] Loaded 3 Mosaic representations (" << mosaic_text_1.size() + mosaic_text_2.size() + mosaic_text_3.size()
    ...
    std::vector<float> host_first(N), host_second(N), host_third(N);
    for (size_t i = 0; i < N; ++i) {
        host_first[i] = static_cast<float>(i);
        host_second[i] = 1.0f;
        host_third[i] = 2.0f;
    }
    ...
    auto upload = [&](const std::vector<float>& host_data)
        -> PJRT_Buffer* {
        int64_t dims[] = {static_cast<int64_t>(N)};
    ...
    PJRT_Buffer* buf_first = upload(host_first);
    PJRT_Buffer* buf_second = upload(host_second);
    PJRT_Buffer* buf_third = upload(host_third);
    if (buf_first == nullptr || buf_second == nullptr || buf_third == nullptr) return 1;
    std::cout << "[7] Uploaded 3 x " << N << " floats to device\n";

    // ---- 9. Execute ---------------------------------------------------
    PJRT_Buffer* output_buffers[1] = {nullptr};
    PJRT_Buffer** output_list = output_buffers;
    PJRT_Buffer* input_args[3] = {buf_first, buf_second, buf_third};
    ...
    std::cout << "\nExpected:\n  ";
    for (int i = 0; i < 8; ++i) {
        float add = host_first[i] + host_second[i];
        float sub = add - host_third[i];
        std::cout << (add * sub) << " ";
    }
    std::cout << "\n";
    ...
    destroy_buffer(buf_first);
    destroy_buffer(buf_second);
    destroy_buffer(buf_third);
